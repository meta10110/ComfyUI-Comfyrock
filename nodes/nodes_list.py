...
class CR_LoadImageList:
    ...
    def make_list(self, start_index, max_images, input_folder, input_path=None):
        ...
        # Calculate the end index based on max_images
        ...
        # Return the list of images as is, without concatenating them
        return (image_list, show_help, )
...
class CR_LoadImageListPlus:
    ...
    RETURN_TYPES = ("IMAGE", "MASK", "INT", "STRING", "STRING", "INT", "STRING", )
    ...
    def make_list(self, start_index, max_images, input_folder, input_path=None, vae=None):
        ...
        index_list = []        
        filename_list = []
        resolution_list = []  # 新增分辨率列表
        
        # Ensure start_index is within the bounds of the list
        start_index = max(0, min(start_index, len(file_list) - 1))

        # Calculate the end index based on max_images
        end_index = min(start_index + max_images, len(file_list))  # 修改这里，去掉-1
        ...
        for num in range(start_index, end_index):
            ...
            # Populate the resolution_list
            width, height = img.size
            resolution_list.append(f"The {num + 1} image: {filename} {width}*{height}")
        ...
        list_length = end_index - start_index  # 返回实际加载的图像数量，而不是路径中的所有图像数量。

        return (image_list, mask_list, index_list, filename_list, resolution_list, list_length, show_help, )
